/*
 Generated Code! Do not manually edit this code. Modify the generator instead.
*/
#pragma once
#include "../../DrEl.h"
#include "../../BnsCustomProperties.h"

namespace BnsTables::KR {

#pragma pack(push, 1)
	struct reinforce_skill_train_by_item_extract_option_Record : BnsTables::Shared::DrEl
	{
	public:
		union Key
		{
            struct {
                __int64 autoId;

            };
			unsigned __int64 key;
		};
		__declspec(align(8)) Key key;
		wchar_t* alias;
BnsTables::Shared::TableRef skill3;
int skill3_tableId() const {return 358;};
__int16 reinforce_score[10];
__int32 reinforce_score_Size() const {return 10;};
signed char reinforce_score_total_count;
char Pad0[3];
BnsTables::Shared::TableRef apply_skill_modify_per_reinforce_score[10];
__int32 apply_skill_modify_per_reinforce_score_Size() const {return 10;};
int apply_skill_modify_per_reinforce_score_tableId() const {return 376;};

		static BnsTables::Shared::TableVersion Version() { return BnsTables::Shared::TableVersion(0, 3); }
		static __int16 TableId() { return 341; }
		static __int32 SubType() { return -1; }

	};
#pragma pack(pop)
#pragma pack(push, 1)
	struct __declspec(align(4)) reinforce_skill_train_by_item_extract_option_RecordPtr // : DrRecordPtr
	{
		reinforce_skill_train_by_item_extract_option_Record* _record;
		int _cacheChunkIndex;
		bool _makeCopy;
	};
#pragma pack(pop)
}